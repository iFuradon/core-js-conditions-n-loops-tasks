...
function getSpiralMatrix(size) {
  ...
  while (topRow <= bottomRow && leftCol <= rightCol) {
    for (let i = leftCol; i <= rightCol; i += 1) {
      startValue += 1;
      matrix[topRow][i] = startValue;
    }
    topRow += 1;
    for (let i = topRow; i <= bottomRow; i += 1) {
      startValue += 1;
      matrix[i][rightCol] = startValue;
    }
    rightCol -= 1;
    if (topRow <= bottomRow) {
      for (let i = rightCol; i >= leftCol; i -= 1) {
        startValue += 1;
        matrix[bottomRow][i] = startValue;
      }
      bottomRow -= 1;
    }
    if (leftCol <= rightCol) {
...
function shuffleChar(str, iterations) {
  ...
  const iter = done ? iterations % cycleLength : iterations;
...
function getNearestBigger(number) {
  ...
  const right = digits.splice(pivot + 1);
  right.sort((a, b) => a - b);
  digits.push(...right);
